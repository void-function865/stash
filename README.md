...
| Fix lightbox landing on the wrong image at a page boundary | [#7082](https://github.com/stashapp/stash/issues/7082) | [#7083](https://github.com/stashapp/stash/pull/7083) | Back the lightbox page-switch guard with a synchronous ref - gating the nav handlers and the index-range clamp, and set before pageCallback - so crossing a gallery page boundary can't land a stale index on the new page's last image. Keeps the landing index handler-controlled (-1 sentinel -> last, 0 -> first, indexInPage for chapters) so first/last wraparound and chapter navigation don't regress. |
